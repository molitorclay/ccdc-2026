...
.SYNOPSIS
    ...
    Sourced from BYU. Slightly modified to fit NCCDC 2026 rule set and clean things up
...
$ccdcRepoWindowsHardeningPath = "https://raw.githubusercontent.com/molitorclay/ccdc-2026/main/windows🪟"
...
# Removed Password Generation & Changing
# Removed "Remove Admin Users"
# Removed "Remove & Add RDP-Users"
$functionNames = @(
    "Initialize Context", "Quick Harden", "Configure Firewall", 
    "Disable Unnecessary Services", "Enable Advanced Auditing", 
    "Configure Splunk", "EternalBlue Mitigated", "Upgrade SMB", "Patch Mimikatz", 
...
function Disable-Unnecessary-Services {
    ...
    Invoke-HardeningOperation -OperationName "Disable Unnecessary Services" -ScriptBlock {
        ...
        # Don't disable IPv6 on AD according as it may cause loss of AD functionality
        # if ($activeAdapters) {
...
function Quick-Harden {
    ...
    Invoke-HardeningOperation -OperationName $operationName -ScriptBlock {
        ...
        # Step 1: Upgrade SMB
        Write-Host "`nStep 1/5: Upgrading SMB..." -ForegroundColor Cyan
        Upgrade-SMB

        # Step 4: Configure Firewall
        Write-Host "`nStep 2/5: Configuring Firewall..." -ForegroundColor Cyan
        Configure-Firewall -FromQuickHarden
        
        # Step 5: Disable unnecessary services
        Write-Host "`nStep 3/5: Disabling Unnecessary Services..." -ForegroundColor Cyan
...
function Show-Main-Menu {
    ...
    Write-Host "  4) Enable Advanced Auditing + Firewall Logging"
    Write-Host "  5) Configure Splunk"
    Write-Host "  6) Install EternalBlue Patch"
    Write-Host "  7) Upgrade SMB (enable v2/3, disable v1)"
    Write-Host "  8) Patch Mimikatz (WDigest)"
    Write-Host "  9) Set Execution Policy to Restricted"
}
